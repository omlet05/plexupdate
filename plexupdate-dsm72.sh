...
# Upgraded by Omlet05 for DSM 7.2.x
#
# Must be run as root.
#
...
jq=$(curl -s ${url})
newversion=$(echo $jq | jq -r '.nas."Synology (DSM 7.2.2+)".version')
...
CPU=$(uname -m)
url=$(echo "${jq}" | jq -r '.nas."Synology (DSM 7.2.2+)".releases[] | select(.build=="linux-'"${CPU}"'") | .url')
